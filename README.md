...
- **定时队列止损与可观测性（[#32](https://github.com/cloader/dsh-taskboard/issues/32)）**：进行中卡片显示排队数，可打开队列查看正在等待派发的任务，并可二次确认清空尚未派发的任务队列；看板同时展示队列最久等待和并发上限。新增可选队列保质期和定时会话启动间隔，默认值保持既有补跑行为。调度器以单飞与派发闸门避免重叠 tick 并行放行队列。
