...
- **执行**：手动、一次性定时或 cron 定期执行（host 侧调度，浏览器关了照跑）；定期任务可选择每轮回到待办，或保留待验收并自动新建下一轮待办卡
...
**第 1 步 · 建卡**：看板右上「+ 新建任务」——选项目、紧急度、执行方式（认领/一次性定时/定期 cron）、模型与 preset、Git 隔离开关、验收清单；定期任务还可选择完成后「移回待办」或「新建待办」；可勾「⚡ 立即执行」。
...
- 新建/编辑弹窗：项目、模型（含思考强度）、紧急度、执行方式、cron 实时校验与下次运行预览、定期任务完成策略、执行隔离开关、验收清单编辑
...
- **定期任务完成策略（0.8.2）**：在任务表单的「定期任务完成后」选择行为。默认「完成后新建待办」会保留本轮待验收，并由宿主创建一张继承 cron、prompt 和配置的新 todo 卡；「完成后移回待办」则把同一张卡复位为 todo，等待下次 cron。选择会存入任务配置，模板与后继卡会继承；宿主结算强制执行，不依赖 agent 自行移卡。未配置的既有 cron 任务同样采用默认的新建待办策略。
...
**定期任务完成后为什么停在「待验收」？如何自动继续？**
默认会保留本轮供验收并由系统创建下一轮 todo 卡。若希望复用同一张卡，在任务表单的「定期任务完成后」选择「完成后移回待办」即可。两种策略不依赖 agent 的状态操作；未配置的既有 cron 任务也会采用默认的新建待办策略。
...
- **定时队列止损与可观测性（[#32](https://github.com/cloader/dsh-taskboard/issues/32)）**：进行中卡片显示排队数，可打开队列查看正在等待派发的任务，并可二次确认清空尚未派发的任务队列；看板同时展示队列最久等待和并发上限。新增可选队列保质期和定时会话启动间隔，后者默认 `1000 ms`，仍可显式设为 `0` 关闭节流。调度器以单飞与派发闸门避免重叠 tick 并行放行队列。
...
- **调度设置**：可在「🛠 设置」配置最大并发执行数（1–100）及「离线错过超时后不补任务」（1–1440 分钟），保存后即时生效，已排队任务不受超时影响。
...
- **执行方式三档**：认领制、定时执行（新增，指定时间只跑一次，触发即消费；错过不补跑）、定期执行（原 cron 定时执行）。
- **定期任务接棒**：每轮执行成功后原卡进入「待验收」供审查，系统自动新建一张 todo 接棒卡承接 cron 继续下一轮周期，验收与周期执行互不阻塞；调度只触发 todo 状态，in_review 卡片不再被定时执行。
