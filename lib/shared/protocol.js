...
function taskAssociatedSessionIds(task) {
	...
	if (typeof task.claimedBy === "string" && task.claimedBy.startsWith("session-")) push(task.claimedBy);
	if (task.createdBy?.kind === "agent" && typeof task.createdBy.sessionId === "string" && task.createdBy.sessionId.startsWith("session-")) push(task.createdBy.sessionId);
	return result;
}
/**
