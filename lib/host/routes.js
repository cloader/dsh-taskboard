...
function registerTaskboardRoutes(ctx, options) {
	...
	let heartbeat;
	const queueSummary = (ledger) => {
		let depth = 0;
		let dispatching = 0;
		let oldestQueuedAt;
		for (const task of ledger.tasks) {
			if (task.execution.queuedRunAt !== void 0) {
				depth += 1;
				if (task.execution.queuedAt !== void 0 && (oldestQueuedAt === void 0 || task.execution.queuedAt < oldestQueuedAt)) oldestQueuedAt = task.execution.queuedAt;
			}
			if (task.execution.dispatchingRunAt !== void 0) dispatching += 1;
		}
		const now = options.now();
		return {
			depth,
			dispatching,
			...oldestQueuedAt === void 0 ? {} : {
				oldestQueuedAt,
				oldestWaitMinutes: Math.max(0, Math.floor((now - oldestQueuedAt) / 6e4))
			},
	...
	*/
	...
	const handler = async (req, res) => {
		try {
			...
			if (pathname === `/dsh-taskboard/queue/clear`) {
				if (options.clearQueue === void 0) {
					json(res, fail("invalid_input", "scheduler queue clear unavailable").res, 501);
					return;
				}
				try {
					json(res, {
						ok: true,
						value: { cleared: await options.clearQueue() }
					});
				} catch (error) {
					const f = toFail(error);
					json(res, f.res, f.status);
