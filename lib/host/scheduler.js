...
var SchedulerService = class {
	...
	dispatchIntervalMs() {
		return typeof this.deps.dispatchIntervalMs === "function" ? this.deps.dispatchIntervalMs() : this.deps.dispatchIntervalMs ?? 1e3;
	}
	/** Start ticking. */
	start() {
		this.disposed = false;
	...
	async tick() {
		if (this.ticking !== void 0) return this.ticking;
		let pass;
		pass = this.tickOnce().finally(() => {
			if (this.ticking === pass) this.ticking = void 0;
		});
		this.ticking = pass;
		return pass;
	}
	...
	isExpired(queuedAt, now) {
		const maxAge = this.queueMaxAgeMs();
		return maxAge > 0 && queuedAt !== void 0 && now - queuedAt >= maxAge;
	}
	...
	async waitForDispatchSlot() {
		...
		await new Promise((resolve) => {
			const handle = this.timers.setTimeout(() => {
				if (this.dispatchWait?.handle === handle) this.dispatchWait = void 0;
				resolve();
			}, wait);
			this.dispatchWait = {
				handle,
	...
	*  overdue shelf-life drop or a manual queue clear from the board. */
	async expireQueued(taskId, queuedWindow, reason = "expired") {
		const now = this.deps.now();
		await this.deps.store.mutate("task-updated", (ledger) => {
			const task = ledger.tasks.find((t) => t.id === taskId);
			if (task === void 0 || task.execution.queuedRunAt !== queuedWindow) return void 0;
			if (reason !== "cleared" && !this.isExpired(task.execution.queuedAt, now)) return void 0;
			delete task.execution.queuedRunAt;
			delete task.execution.queuedAt;
			task.comments.push({
				id: newCommentId(),
				body: normalizeBody(reason === "cleared" ? "[系统] 已被手动清出执行队列，本次不再补跑；可手动执行或修改定时。" : "[系统] 排队中的定时执行已超过保留时长，本次不再补跑；可手动执行或修改定时。"),
	...
	async clearQueue() {
		...
		await this.deps.store.mutate("task-updated", (ledger) => {
			...
			return changed.length === 0 ? void 0 : changed;
		});
		return cleared;
	}
