import { asIsolation, asPermission, asStatus, asUrgency, canTransition, checklistFromTexts, defaultIsolationOf, defaultPermissionOf, effectivePrompt, isClaim, isClaimedBy, newCommentId, newTaskId, normalizeBody, normalizeExecution, normalizeExecutionReport, normalizeModel, normalizePrompt, normalizeTitle, summarize, syncClaim } from "../shared/protocol.js";
...
function taskDetail(t) {
	const lines = [
		...
		`执行方式: ${t.execution.mode}${t.execution.cron !== void 0 ? ` 定期 cron=${t.execution.cron}` : ""}${t.execution.runAt !== void 0 ? ` 定时(一次) runAt=${new Date(t.execution.runAt).toISOString()}` : ""}`,
...
function registerTaskboardTools(ctx, deps) {
	...
	disposers.push(register(defineTool({
		...
		parameters: {
			...
			execution: {
				...
				properties: {
					...
					cron: {
						type: "string",
						description: "Five-field cron expression (scheduled periodic only)."
					},
					runAt: {
						type: "string",
						description: "One-shot trigger time: epoch ms or ISO string (scheduled one-shot only; must be in the future)."
					}
				}
			},
			model: {
		...
		async execute(args, exec) {
			try {
				...
				const permission = args.permission === void 0 ? defaultPermissionOf(store.snapshot().settings) : asPermission(args.permission);
	...
	disposers.push(register(defineTool({
		...
		description: "Update a task's title/description/prompt/urgency/blocked/permission. Requires ifVersion (read first). The model and execution config are read-only through this tool (they belong to the task owner/user).",
		parameters: {
			id: {
				type: "string",
				required: true,
			...
			permission: {
				type: "string",
				description: "Execution permission: read-only | workspace-write | danger-full-access."
			}
		},
		output: {
			schema: JSON_OUT,
		...
		async execute(args, exec) {
			try {
				...
				await store.mutate("task-updated", (ledger) => {
					...
					if (args.permission !== void 0) next.permission = asPermission(args.permission);
