...
function workspaceFace(registry) {
	return {
		...
		archiveSession: async (sessionId) => {
			const reg = registry;
			if (typeof reg.archiveSession === "function") try {
	...
	disposers.push(register(defineTool({
		...
		async execute(args, exec) {
			try {
				...
				if (to === "archived" && args.archiveSessions === true && deps.workspaces.archiveSession !== void 0) {
					const targetTask = beforeTask ?? next;
					if (targetTask !== void 0) {
						const sessionIds = taskAssociatedSessionIds(targetTask);
						for (const sid of sessionIds) try {
							await deps.workspaces.archiveSession(sid);
						} catch {}
					}
				}
				return json({ task: summarize(next) });
