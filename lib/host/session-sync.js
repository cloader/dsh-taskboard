import { defaultSyncExternalSessionsOf, newCommentId, newExecutionId, newTaskId, nextCronTime, normalizeBody, normalizeTitle, parseCron, spawnNextCycle } from "../shared/protocol.js";
...
var ExternalSessionSyncService = class {
	...
	*/
	...
	async handleSessionEvent(sessionId, event, sessionMeta) {
		...
		const now = this.deps.now();
		if (event.type === "turn/start" || event.type === "user/message" || event.type === "turn/step" || event.type === "turn/progress" || event.type === "agent/step" || event.type === "agent/thought") await this.touchActivity(sessionId, now);
	...
	async touchActivity(sessionId, now) {
		...
		await this.deps.store.mutate("execution-recorded", (ledger) => {
			const task = ledger.tasks.find((current) => current.claimedBy === sessionId || current.executions.some((execution) => execution.sessionId === sessionId && execution.outcome === "running"));
			const execution = task === void 0 ? void 0 : [...task.executions].reverse().find((current) => current.sessionId === sessionId && current.outcome === "running");
